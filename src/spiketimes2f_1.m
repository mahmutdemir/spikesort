...
function [f,t] =  spiketimes2f_1(spiketimes,time,dt,window,algo)
...
switch algo
    ...
    case 'gaussCausal'
        ...
        gw = gausswin(length(tt),1/window);
        
        
        %         window = round(2*window/deltat); % ms
        %         gw = gausswin(window);
        
        if iseven(length(gw))
            gw = gw(1:length(gw)/2);
        else
            gw = gw(1:(length(gw)+1)/2);
        end
        %         gw = gw/sum(gw);
        
        f = zeros(length(t),ntrials);
        for i = 1:ntrials
            %             ff = conv(full(spiketimes(:,1)),gw,'same')*length(gw)/(window/2)*1000;
            %             ff = ff(1:length(spiketimes(:,1))); % remove the padded end
            
            ff = filter(gw,1,full(spiketimes(:,i)));
